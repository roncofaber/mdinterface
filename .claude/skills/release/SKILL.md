...
it dynamically). Bump it to `X.Y.Z`, no `v` prefix.
...
In that order. Pushing the tag triggers `.github/workflows/release.yml`:
`preflight` → `test` → `build` → `publish` (PyPI, automatic) → `release` (draft GitHub release,
notes = changelog summary + link to CHANGELOG.md).
