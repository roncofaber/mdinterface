...
### Changed
- Release process: `publish.yml` replaced by `release.yml` - now runs the test suite and requires
  a changelog summary paragraph before publishing to PyPI, and creates a draft (not published)
  GitHub release with changelog-sourced notes instead of autogenerated commit notes
- `.github/workflows/release.yml` also accepts manual `workflow_dispatch` (with a `tag` input) for
