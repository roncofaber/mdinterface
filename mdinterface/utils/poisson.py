...
def integrate_poisson_1D(
    r, charge_dens_profile, periodic=False, return_efield=False, rule="trapezoid"
):
    ...
    if periodic:
        ...
        Vfield = Vfield - (
            (Vfield[-1] - Vfield[0]) / (r[-1] - r[0]) * (r - r[0]) + Vfield[0]
        )
...
class Poisson1D_bvp:
    ...
    def solve(self, rho_at_z, phi, pd_iter):
        ...
        rho = sint.interp1d(
            self._z_grid, rho_at_z, kind="cubic", fill_value="extrapolate"
        )
