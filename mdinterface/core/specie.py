...
from mdinterface.externals import (
...
class Specie:
    ...
    def _setup_topology(
        ...
        dihedrals_list, dihedral_map, dihedral_ids = pmap.map_dihedrals(
            as_list(dihedrals)
        )
        impropers_list, improper_map, improper_ids = pmap.map_impropers(
            as_list(impropers)
        )
        ...
        if fix_missing:  # ugly but seems to work
            ...
            angles_list, angle_map, angle_ids = pmap.map_angles(
                as_list(angles) + mss_ang
            )
            dihedrals_list, dihedral_map, dihedral_ids = pmap.map_dihedrals(
                as_list(dihedrals) + mss_dih
            )
            impropers_list, improper_map, improper_ids = pmap.map_impropers(
                as_list(impropers) + mss_imp
            )
    ...
    def round_charges(self, nround=7):
        ...
        rcharges = round_list_to_sum(
            self.charges, round(sum(self.charges), nround), nround
        )
    ...
    def add_topology(self, attribute):
        ...
        for existing_attribute in attribute_list:
            if existing_attribute.symbols == attribute_symbols:
                print(
                    f"{attribute_type} with symbols {attribute_symbols} already exists and will not be added again."
                )
                return
    ...
    def to_universe(self, charges=True, layered=False, match_cell=False, xydim=None):
        ...
        # layer it nicely
        if layered:
            layer_idxs, __ = ase.geometry.get_layers(
                self.atoms, [0, 0, 1], tolerance=0.01
            )
            groups = []
    ...
    def find_relevant_distances(self, Nmax, Nmin=0, centers=None, Ninv=0):
        ...
        unique_pairs_list = find_relevant_distances(
            self.graph, Nmax, Nmin=Nmin, centers=centers, Ninv=Ninv
        )
