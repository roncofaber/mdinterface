...
class Polymer(Specie):
    ...
    def __init__(
        ...
        nrep=1,
        start_end_idxs=None,
        target_distance=1.600,
        substitute=None,
        refine_charges=False,
        offset=False,
    ):
    ...
    def make_snippet(self, centers, Nmax, ending="F"):
        ...
        idxs = list(
            set(np.concatenate(self.find_relevant_distances(Nmax, centers=centers)))
        )
        edxs = list(
            set(
                np.concatenate(
                    self.find_relevant_distances(Nmax + 1, centers=centers, Nmin=Nmax)
                )
            )
            - set([centers])
        )
    ...
    def _update_charges(self, center, Nmax, charges, ending="F"):
        ...
        ldxs = list(
            set(np.concatenate(self.find_relevant_distances(3, centers=center)))
        )
