...
# package stuff
from mdinterface.utils.auxiliary import as_list, atoms_to_indexes
...
class Trajectory(object):
    ...
    def __init__(
        ...
        self._traj, self._data = self._read_trajectory(
            filename, datafile, index, parallel=parallel, every=every
        )
    ...
    def _setup_electrode(self, electrode_atoms):
        ...
        self._int_idxs = idxs[
            np.where(
                self[0].get_positions(wrap=True)[idxs][:, self._zaxis] < self.depth / 2
            )[0]
        ]
        self._end_idxs = idxs[
            np.where(
    ...
    def get_2D_density(
        ...
        if zmax is None:
            zmax = [
                self[0].get_cell()[ax, ax] for ax in axes
            ]  # Get the z-dimension of the cell
        ...
        for frame in self:
            ...
            if zrange is not None:
                zmask = np.where(
                    np.logical_and(
                        pos[:, zaxis] >= zrange[0], pos[:, zaxis] <= zrange[1]
                    )
                )[0]
            else:
                zmask = slice(None)
            ...
            if charge is None:
                density.append(
                    np.histogram2d(
                        pos[zmask, axes[0]], pos[zmask, axes[1]], bins=z_bins
                    )[0]
                )
            else:
        ...
        return (
            (z_bins[0][:-1] + z_bins[0][1:]) / 2,
            (z_bins[1][:-1] + z_bins[1][1:]) / 2,
            density,
        )

    def get_1D_potential(
        self,
        axis=None,
        nbins=100,
        symbols="all",
        ...
        zpos, chgden = self.get_1D_density(
            axis=axis,
            nbins=nbins,
            symbols=symbols,
            charge=True,
        ...
        if correct_background and not return_mean:
            bkg_chg = (
                np.array([frame.get_initial_charges().sum() for frame in self])
                / self.cell.volume
            )
        ...
        pot = integrate_poisson_1D(
            zpos, chgden, periodic=periodic, rule=rule, return_efield=return_efield
        )
    ...
    def get_potential_difference(
        ...
        # get 1D potential
        zpos, pot = self.get_1D_potential(
            nbins=2000,
            symbols="all",
            return_mean=False,
            periodic=periodic,
    ...
    def get_dV_on_electrodes(
        ...
        return self.get_potential_difference(
            st_crd,
            en_crd,
            nbins=nbins,
            axis=axis,
    ...
    def get_coordination_number(
        self, ref_atom, tar_atom, r0, frames=slice(None), n=6, m=12, a=None
    ):
